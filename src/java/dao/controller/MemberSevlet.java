...
public class MemberSevlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet MemberSevlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet MemberSevlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        ArrayList<EventParticipants> per = EventParticipantDAO.getEventParticipationOne(user.get(0).getUserID());
        PrintWriter out = response.getWriter();

        if (per != null) {
                request.setAttribute("hismem", per);
